...
impl State {
    ...
    pub(crate) fn remote_peer_id(&self, connection_id: ConnectionId) -> Option<&PeerId> {
    ...
    pub(crate) fn handle_event<R: rand::Rng>(
        ...
        match event {
            ...
            HubInput::Command {
                ...
                if let Some(result) = self.handle_command(rng, now, results, command_id, *command) {
            ...
            HubInput::ActorMessage { actor_id, message } => {
                match message {
                    ...
                    DocToHubMsgPayload::SendSyncMessage {
                        ...
                        if let Some((conn, target_id)) = self.established_connection(connection_id)
                        {
